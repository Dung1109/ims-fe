...
const publicRoutes = ['/login', '/register', '/postlogin']
...
export function middleware(request: NextRequest) {
  ...
  if (authCookie) {
    try {
      ...
      const authData = typeof cookieValue === 'string' ? JSON.parse(cookieValue) : cookieValue

      // Check for authenticated status
      isAuthenticated = authData.state?.authenticated || authData.authenticated || false
