...
async function fetchUsers(
  ...
  search: string,
  role: string
): Promise<{ content: UserinfoResponseDTO[]; totalPages: number }> {
  const res = await fetch(
    `http://127.0.0.1:8080/resource-server/users?pageNo=${page}&pageSize=${size}&filterBy=${search}&filterRole=${role}`,
    {
      method: "GET",
      credentials: "include",
      headers: {
