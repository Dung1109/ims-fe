...
export default function EmployeeList() {
    ...
    const fetchEmployees = async () => {
        try {
            ...
            const response = await fetch(
                `http://127.0.0.1:8080/employee-resource-server/employee?${queryParams}`,
                {
                    credentials: "include",
                }
            );
            ...
            setEmployees(data.employees.map((employee: any) => ({
                id: employee.id.toString(),
                name: employee.name,
                email: employee.email,
                phoneNo: employee.phoneNo,
                currentPosition: employee.currentPosition,
                ownerHR: employee.ownerHR,
    ...
    const handleDelete = async (id: string) => {
        try {
            ...
            const response = await fetch(
                `http://127.0.0.1:8080/employee-resource-server/employee/${id}`,
                {
                    method: "DELETE",
                    credentials: "include",
                    headers: {
                        ...
                        <tbody>
                            {filteredEmployees.map((employee) => (
                                <tr key={employee.id} className="border-t">
                                    ...
                                    <td className="p-4">{employee.phoneNo}</td>
                                    <td className="p-4">
                                        {employee.currentPosition}
                                    </td>
                                    <td className="p-4">{employee.ownerHR}</td>
                                    <td className="p-4">
                                        <span
